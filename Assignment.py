...
class CSP:
    ...
    def is_complete(self, assignment):
            return all(len(assignment[var]) == 1 for var in assignment)
    ...
    def backtrack(self, assignment):
        ...
        if self.is_complete(assignment):
            ...
            print("Solution found with", self.call_count, "calls to backtrack and", self.fail_count, "failures")
            print("Solution:")
            print_sudoku_solution(assignment)
            return assignment

        # Choose the unassigned variable with the fewest legal values
    ...
    def revise(self, assignment, i, j):
        ...
        for x in assignment[i]:
            if not any(y != x for y in assignment[j]):
...
def get_solution():
    easy_csp = create_sudoku_csp('easy.txt')
    easy_csp.backtracking_search()

    medium_csp = create_sudoku_csp('medium.txt')
    medium_csp.backtracking_search()

    hard_csp = create_sudoku_csp('hard.txt')
    hard_csp.backtracking_search()

    very_hard_csp = create_sudoku_csp('veryhard.txt')
    very_hard_csp.backtracking_search()
